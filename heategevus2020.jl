...
# ╔═╡ 3ff6e3ac-f6fa-414e-bc4b-009dc1b1eed8
begin
	weight_A = [MetaGraphs.weights(G_kood)[x, y] for x=1:nv(G_kood), y=1:nv(G_kood)]
	A = adjacency_matrix(G_kood) .* weight_A .* (ones(991, 991) - I) |> normalize
end

# ╔═╡ 29d1129f-2b62-4831-8efc-76c7f5869302
begin	
	Y = sgtsnepi(A; d=3, λ=10 )
end
...
# ╔═╡ 5c660eb6-4df9-44e0-ae95-0e993a7ca003
e1 = collect(edges(sg))

# ╔═╡ 8353e29f-8eb1-4997-b3f3-3fbf7bffc771
edge_weights = normalize([get_prop(sg, edge, :weight) for edge=edges(sg)])

# ╔═╡ a17e701a-9d72-4252-b2ca-524570cc54d7


# ╔═╡ 939f4fd6-4f2c-4d68-896d-51a25d43f249
[ neighbors(sg, node) for node = vertices(sg) ]
...
# ╔═╡ de3154a1-3264-48d3-9b0c-65e31c1dbbc1
...
graphplot(sg, nlabels=glabel[1:topN])

# ╔═╡ 20db2ca6-cee6-44e4-aadc-a095e9b4fefc
text_positions = [(x,y,z) for (x,y,z) in eachrow(Y)]

# ╔═╡ 4609a7f0-d2ba-402e-a22e-c69437e2a57f
begin
	
	fig = show_embedding(Y; A=A, lwd_in = 0.4, lwd_out = 0.01, res=(2600, 2600), mrk_size=8)
	text!(glabel, position=text_positions, align = (:right, :center), offset = (-20, 0), textsize=10)
	fig
end

# ╔═╡ 6e743b47-5f2b-4128-b481-13b5d6ab59e7
function show_embedding3(
  ...
  function _plot_lines!( ax, Y, i, j, idx, color, lwd )
	...
	ll = vec( [reshape( Y[ vcat( i[idx], j[idx] ), 3 ], (Int32.(sum(idx)), 2) ) NaN*zeros(sum(idx))]' )

    lines!( ax, ii, jj, ll, color = color, linewidth = lwd )
  ...
  scatter!(ax, Y[:,1], Y[:,2], Y[:,3], color = L, colormap = cmap,
           markersize = mrk_size);
...
# ╔═╡ 2396017a-13ff-45c6-b683-9b5e69a6ed75
