...
# ╔═╡ 20661b40-9e03-4e90-ada7-64a9e25e1074
using NetworkLayout

# ╔═╡ 5edfcb3f-b9c5-4541-811a-de5d2493f11d
using GraphPlot
	...
	for person_jrk = get_prop(G_kood, i, :jrknrs)
		for (kood_s, summ) = get_prop(G_inim, G_inim[person_jrk, :JRKNR], :kood_sum)
			...
			if !has_edge(G_kood, i, edge_to)
				add_edge!(G_kood, i, edge_to, :summa, summ)
			else
				prev_sum = get_prop(G_kood, i, edge_to, :summa)
				set_prop!(G_kood, i, edge_to, :summa, prev_sum + summ)
			end
		end
	end
 end

# ╔═╡ 69bfcd7b-1d4c-4413-bc5d-4224bc29783b
gplot(G_kood)
...
git-tree-sha1 = "aa7689c09728b84ef41cfb7ae0f21fcff9725b48"
...
git-tree-sha1 = "6405eb2101e93a9d5f77af8c2b5b5986fa303578"
