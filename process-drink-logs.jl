...
# ╔═╡ 3684f6bc-d555-11ec-04e7-af32e442f354
begin
	using HTTP
	using JSONTables
	using JSON3
...
# ╔═╡ a9e8c8f7-6ec2-432c-8674-c9de52931ccf
set_aog_theme!()

# ╔═╡ e0a61f22-d140-4758-9b9c-49a0f65b54c9
update_theme!(markersize=10,)

# ╔═╡ ddcfcb58-3bdf-47e2-b90b-e60001d37625
auth = HTTP.post("https://prod.yanu.ai/api/login", [], HTTP.Form(Dict("username" => "admin", "password" => "admin")));

# ╔═╡ 79a9ef76-cb90-4d99-aa4d-b5860566fb60
auth_token = JSON3.read(auth.body).data.token

# ╔═╡ d099104d-e265-44b8-9d4f-686a41f9b6d4
headers = [
	"accept" => "application/json",
	"Authorization" => "Bearer $auth_token"
]

# ╔═╡ 05a616fd-3e15-444c-b867-5d1dc46f5102
r = HTTP.get("http://prod.yanu.ai/api/v1/drink_orders?pagination=false", headers);

# ╔═╡ 2adff0de-b10a-488a-981f-b847d10c6e98
function to_dt(str)
	notz = first(rsplit(str, ['+','-'], limit=2))
	DateTime(notz)
end
	

# ╔═╡ fbd31e09-0fb0-4a70-8f00-b912eb2b301c
df = @chain DataFrame(jsontable(r.body)) begin
	transform([:createdAt => ByRow(to_dt) => :createdAt, 
		:updatedAt => ByRow(to_dt) => :updatedAt,
		:drink => ByRow(x -> x.name) => :drink
	])
	

end

# ╔═╡ c99e2909-e856-4cfb-98a0-e30b161209e0
@chain df begin
	subset(:createdAt => ByRow(x -> Date(x) == Date(2022,05,16)))
	subset(:xStatusId => x -> x .== "DELIVERED")
end

# ╔═╡ 354c245c-f7d1-4136-90fd-1acb260c0f48
@chain df begin
	subset(:createdAt => ByRow(x -> Date(x) == Date(2022,05,17)))
	subset(:xStatusId => x -> x .== "DELIVERED")
end

# ╔═╡ a8751083-346c-4801-98ae-b1a2462869a2
let 
	df = @chain df begin
		subset(:createdAt => ByRow(x -> Date(x) >= Date(2022,05,15)))
		transform(:createdAt => ByRow(Day) => :day)
	end
	plt = data(df) *
  mapping(:createdAt, :id, color=:drink, marker=:drink, row=:day) * 
  visual(Scatter, alpha=0.5)

	draw(plt, facet = (; linkxaxes = :none, linkyaxes = :none))
end

# ╔═╡ d5f15919-269b-4c95-b259-bd189339b5b4
@chain df begin
	subset(:createdAt => ByRow(x -> Date(x) >= Date(2022,05,15)))
	transform(:createdAt => ByRow(Day) => :day)
	groupby(:day)
...
# ╔═╡ 5f0f1cc3-30ad-416d-a38b-9b372f252deb
...
let 
	...
	plt = data(df) *
  		mapping(:drink, :nrow, color=:drink, col=:day) * 
  		visual(BarPlot)
