...
# ╔═╡ ed3ad31e-ce43-11ed-0341-9f322b7d0643
begin
	using Pkg
	using PlutoUI
	using StatsBase
...
# ╔═╡ 1c3bd7fd-f399-4a27-ada6-f7e19e9f7b58
df_orig = CSV.read("/home/kris/Downloads/age-willing-to-date-survey-anonymous.csv", DataFrame)

# ╔═╡ 7c63556a-7777-4f90-8ea7-0e9b7adc774f
df = @chain df_orig begin
	select(Not(28:2:56))
	select(Not([3,4,9,10,11,12, 14, 16, 20, 21, 24]))
	dropmissing([:age])
	transform(:age => ByRow(x -> round(Int, x)) => :age)
end

# ╔═╡ 8ff53203-d462-4889-9bb1-061b1c978a5b
countmap(df.country)

# ╔═╡ 3c965de1-39ad-4ab9-876c-eb306e230444
df_est = @chain df begin
	subset(:country => x -> x .== "Estonia"; skipmissing=true)
	# data(_)
end

# ╔═╡ 4a3e27c2-3016-460e-89f7-9c6c1ea7c4df


# ╔═╡ 0a872273-8d34-4dec-af67-e9957d82e085
names(df_orig)

# ╔═╡ 57e879de-53b6-4605-96f2-3928cd74101d
select(df_est, :age, 17:31)

# ╔═╡ 685807c3-2e41-4f2d-a6f5-079031c61b42
collect(4:2:12)

# ╔═╡ 2a9a66c8-a248-4895-8ec0-2d86408dcf4b
names(df)

# ╔═╡ d61f8799-e52a-448c-88e2-b5da5d44e8ac
let plt = data(df_est) * frequency() * mapping(:years0, color=:age, dodge=:age)
	draw(plt, axis=(xticks = LinearTicks(12), title="Annetajate arv aastal 2021"),)
end
...
manifest_format = "2.0"
project_hash = "53f13805e1b49924b6c00bee7c38439feda1ea50"
